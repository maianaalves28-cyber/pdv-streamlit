...
def processar_codigo_barras(codigo_barras, peso_manual, produtos_data):
    ...
    try:
        ...
        if len(codigo_barras) == 13 and codigo_barras.startswith('2'):
            ...
            # Extrair os 5 dígitos do peso em gramas: posições 7 a 11 (índices 6 a 10)
            peso_raw = codigo_barras[6:11] # Peso em gramas (5 dígitos)

            st.write(f"DEBUG: Produto Código Extraído (índices 2:6): {produto_cod}")
            st.write(f"DEBUG: Peso Raw Extraído (índices 6:11): {peso_raw}")
            ...
            st.write(f"DEBUG: Caracteres na fatia [6:11]: {debug_chars}")
