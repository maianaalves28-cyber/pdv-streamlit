...
def processar_codigo_barras(codigo_barras, peso_manual, produtos_data):
    ...
    try:
        ...
        if len(codigo_barras) == 13 and codigo_barras.startswith('2'):
            produto_cod = codigo_barras[1:5] # Código do produto (4 dígitos)
            # CORREÇÃO BASEADA NO EXEMPLO: Se 2019200005980 (produto 1920) é 1.216 kg,
            # então os dígitos do peso devem representar 1216 gramas.
            # No código 2019200005980, os dígitos 7 a 10 (índices 6 a 9) são '0005'.
            # Isso ainda não bate. Vamos tentar extrair os 5 dígitos do peso a partir de outra posição.
            # Se o peso é 1216 gramas, e o código é 2019200005980, talvez os 5 dígitos
            # do peso sejam representados de outra forma.
            # Tentativa 1: Extrair 5 dígitos a partir do índice 6
            # peso_raw = codigo_barras[6:11] # Resulta em '00598' -> 598 gramas
            # Tentativa 2: Extrair 5 dígitos a partir do índice 7
            # peso_raw = codigo_barras[7:12] # Resulta em '05980' -> 5980 gramas
            # Tentativa 3: Se 1.216 kg = 1216 gramas, e o código é 2019200005980
            # Onde estão os dígitos '1216'? Eles não parecem estar juntos.
            # Pode ser que o peso utilize 4 dígitos em vez de 5, ou a estrutura é diferente.

            # Vamos retornar à estrutura comum de 5 dígitos para o peso, mas garantir a extração correta.
            # Toledo Prix padrão: 2 + AAAA + BBBBB + C + D
            # AAAA = Código do item (4 dígitos)
            # BBBBB = Peso (5 dígitos em gramas)
            # C = Dígito verificador do peso
            # D = Dígito verificador total
            # No seu código 2019200005980:
            # 20 -> Prefixo
            # 1920 -> Código do item
            # 00005 -> Peso em gramas?? (Isso não faz sentido se o peso é 1.216 kg)
            # 9 -> Dígito verificador do peso?
            # 80 -> Dígitos verificadores finais?

            # Há uma inconsistência entre o código fornecido e o peso esperado (1.216 kg = 1216 g).
            # Se o código 2019200005980 realmente representa 1.216 kg, a estrutura pode ser diferente ou
            # o peso é codificado em 4 dígitos ou com um fator diferente.

            # Vamos tentar uma extração alternativa para ver se encontramos '1216' ou '1.216'
            # Se o peso é 1.216, talvez os 5 dígitos sejam '12160' (dividido por 10000) ou '01216' (dividido por 1000)

            # Assumindo que os 5 dígitos do peso estão entre os índices 5 e 10 (posição 6 a 10)
            peso_raw = codigo_barras[5:10] # No seu exemplo, '00005'
            peso = int(peso_raw) / 1000.0 # Resulta em 0.005 kg

            # Isso não corresponde a 1.216 kg.
            # Vamos tentar outra extração baseada na possibilidade de o peso estar em posições diferentes.
            # Se o peso é 1.216, que são 1216 gramas. Onde '1216' aparece no código? Não aparece diretamente.
            # E se o peso for codificado em 4 dígitos em vez de 5?
            # 2 + PPP P + WWWW + DD (2 + 4 produto + 4 peso + 2 verificadores)
            # Nesse caso, os 4 dígitos de peso estariam em posições como 6-9 (índices 5-8) ou 7-10 (índices 6-9).
            # Se estiverem em 7-10 (índices 6-9), em 2019200005980, seria '0000'. Ainda não é 1216.

            # Vamos voltar à extração original de 5 dígitos, mas reconsiderar o divisor.
            # Se 00005 gramas resultou em 121 gramas (ou 0.121 kg), talvez o divisor não seja 1000.
            # Mas a estrutura padrão Toledo Prix para peso é 5 dígitos em gramas / 1000 para kg.

            # Dada a inconsistência, vou manter a extração padrão (índices 5 a 10 para peso_raw)
            # mas é possível que a sua balança use um padrão ligeiramente diferente,
            # ou que o exemplo de código de barras fornecido represente um peso diferente.

            # Revertendo para a lógica que extrai 5 dígitos para o peso (índices 5 a 10)
            peso_raw = codigo_barras[5:10] # Extrai 5 dígitos para o peso em gramas
            peso = int(peso_raw) / 1000.0 # Converte para kg

            # Se o problema persistir, precisaremos confirmar o padrão exato de codificação da sua balança.
