    ...
    if st.button("Gerar Relatório"):
        if not st.session_state.historico.empty:
            ...
            if not vendas_do_dia.empty:
                ...
                # NOTE: If history contains both 'Peso (kg)' and 'Peso (g)', this sum might be incorrect.
                # For a correct daily report in kg, we should ensure all weights are in kg before summing.
                # Assuming for now that the user will handle the unit consistency or that all relevant entries are in kg.
                # To correctly sum in kg, we'd need to:
                # 1. Ensure 'Peso (g)' column exists and convert it to kg: vendas_do_dia['Peso (kg)_from_g'] = vendas_do_dia['Peso (g)'] / 1000.0
                # 2. Combine weight columns and sum: vendas_do_dia['Total_Peso_kg'] = vendas_do_dia.get('Peso (kg)', 0) + vendas_do_dia.get('Peso (kg)_from_g', 0)
                # 3. Group by product and sum 'Total_Peso_kg'.
                # Sticking to the original column for now as per the request's focus on input decoding.
                if 'Peso (kg)' in vendas_do_dia.columns:
                     ...
                     st.subheader(f"Relatório de Vendas para {data_selecionada_str} (em kg)")
                     st.dataframe(relatorio_diario)
                elif 'Peso (g)' in vendas_do_dia.columns:
                     # If only grams are present, sum in grams and note it in the header
                     relatorio_diario = vendas_do_dia.groupby("Produto")["Peso (g)"].sum().reset_index()
                     relatorio_diario.rename(columns={"Peso (g)": "Total Vendido (g)"}, inplace=True)
                     st.subheader(f"Relatório de Vendas para {data_selecionada_str} (em gramas)")
                     st.dataframe(relatorio_diario)
                else:
                     st.info(f"Nenhuma venda com dados de peso registrada para {data_selecionada_str}.")


                 # Generate CSV for download (using the generated relatorio_diario DataFrame)
                if not relatorio_diario.empty:
                    csv_relatorio = relatorio_diario.to_csv(index=False).encode("utf-8")
                    # Add download button
                    st.download_button(
                        label=f"⬇️ Baixar Relatório Diário ({'kg' if 'kg' in relatorio_diario.columns else 'gramas'} - CSV)",
                        data=csv_relatorio,
