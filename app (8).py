...
def processar_codigo_barras(codigo_barras, peso_manual, produtos_data):
    ...
    try:
        ...
        if len(codigo_barras) == 13 and codigo_barras.startswith('2'):
            produto_cod = codigo_barras[1:5] # Código do produto (4 dígitos) - indices 1 a 4
            # Extrair os 5 dígitos do peso em gramas: posições 6 a 10 (índices 5 a 9)
            peso_raw = codigo_barras[5:10] # Peso em gramas (5 dígitos)

            st.write(f"DEBUG: Código de barras 13 digitos detectado: {codigo_barras}")
            st.write(f"DEBUG: Produto Código Extraído: {produto_cod}")
            st.write(f"DEBUG: Peso Raw Extraído (índices 5:10): {peso_raw}")
