...
def processar_codigo_barras(codigo_barras, peso_manual, produtos_data):
    ...
    try:
        ...
        # Caso 1: Código da balança (13 dígitos) - Decodifica padrão com peso incluindo o 13º dígito
        ...
        # Com base na informação de que o 13º dígito (índice 12) faz parte do peso,
        # e no exemplo 2019200005980 (peso 0.598 kg), vamos assumir que os 5 dígitos do peso
        # estão nos índices 8 a 12 (posições 9 a 13).
        # No exemplo, codigo_barras[8:13] seria '05980'.
        # Se '05980' representa 0.598 kg, então 05980 / 10000 = 0.598.

        if len(codigo_barras) == 13 and codigo_barras.startswith('2'):
            st.write(f"DEBUG: Código de barras completo detectado: {codigo_barras}") # Print the full string
            st.write(f"DEBUG: Tipo do código de barras: {type(codigo_barras)}") # Print the type
            st.write(f"DEBUG: Comprimento do código de barras: {len(codigo_barras)}") # Print the length

            produto_cod = codigo_barras[2:6] # Código do produto (4 dígitos) - indices 2 a 5
            # Extrair os 5 dígitos do peso: posições 9 a 13 (índices 8 a 12)
            peso_raw = codigo_barras[8:13] # Peso (5 dígitos)

            st.write(f"DEBUG: Produto Código Extraído (índices 2:6): {produto_cod}")
            st.write(f"DEBUG: Peso Raw Extraído (índices 8:13): {peso_raw}")

            # Debugging adicional: imprimir caracteres individuais na fatia do peso
            debug_chars = ""
            for i in range(8, 13):
                if i < len(codigo_barras):
                    debug_chars += f"codigo_barras[{i}]: '{codigo_barras[i]}' "
                else:
                    debug_chars += f"codigo_barras[{i}]: Index out of bounds "
            st.write(f"DEBUG: Caracteres na fatia [8:13]: {debug_chars}")


            # Converte para kg, dividindo por 10000.0 para 3 casas decimais (ex: 05980 -> 0.598)
            peso = int(peso_raw) / 10000.0

            st.write(f"DEBUG: Peso Calculado (kg): {peso}")
