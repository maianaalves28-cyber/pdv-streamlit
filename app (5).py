...
def processar_codigo_barras(codigo_barras, peso_manual, produtos_data):
    ...
    try:
        ...
        if len(codigo_barras) == 13 and codigo_barras.startswith('2'):
            ...
            # ALTERAÇÃO: Interpretando os 5 dígitos do peso diretamente como quilogramas.
            # Isso assume que a balança codifica o peso diretamente em kg com 3 casas decimais implícitas.
            peso_raw = codigo_barras[5:10] # Peso em kg (5 dígitos)
            # Se o peso for 1.216 kg, peso_raw seria '12160'. Se for 0.500 kg, seria '05000'.
            # Vamos dividir por 10000 para obter o valor correto em kg se a última casa for decimal.
            peso = int(peso_raw) / 10000.0 # Converte para kg (assumindo 4 casas decimais implícitas no raw, mas só 3 relevantes)
...
with st.expander("⬆️ Gerenciar Produtos"):
    ...
    if st.session_state.produtos:
        ...
        if st.button("Excluir Produtos Selecionados", key="excluir_produtos_button"):
            excluir_produtos_seleicionados()
