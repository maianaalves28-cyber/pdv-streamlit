...
def processar_codigo_barras(codigo_barras, peso_manual, produtos_data):
    ...
    try:
        ...
        # Caso 1: Código da balança (13 dígitos) - Decodifica padrão Toledo Prix (assumindo '2' + 4 digitos produto + 5 digitos peso + 1 digito verificador)
        if len(codigo_barras) == 13 and codigo_barras.startswith('2'):
            produto_cod = codigo_barras[1:5] # Código do produto (4 dígitos após o '2')
            peso_raw = codigo_barras[6:11] # Peso em gramas (5 dígitos, ignora dígito verificador da balança)
            peso = int(peso_raw) / 1000.0 # Converte para kg
            # Note: The check digit is not used for product/weight extraction in this standard,
            # but could be validated if needed. The current logic assumes the structure.
